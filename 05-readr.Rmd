...
用于导入数据的函数一般都有大量的参数，上述的三个函数中，比较重要的参数有：
...
`readr` 是 `tidyverse` 的核心 R 包之一，所以我们不必在手动加载它，其中的多数函数能够将平面文件(flat files)转换为数据框。相对于R基础包中有同样功能的函数，它有以下的优点：  

* 一般来说，它们比基础模块中的函数速度更快（约快 10 倍）。如果只考虑速度的话，还可以尝试使用 `data.table::fread()`。这个函数与 `tidyverse` 的兼容性不是很好，但确实更快一些   
* 它们可以生成`tibble`，而且不会将字符串向量转换为因子，不使用行名称，也不会随意改动列名称。这些都是使用 R 基础包时比较烦人的事情  
* 它们更容易重复使用。R基础包中的函数会继承操作系统的功能，并依赖环境变量，因此，可以在你的计算机上正常运行的代码未必适用于他人的计算机  
...
解析数值似乎是直截了当的，但以下3个问题增加了数値解析的复杂性：  
...
`parse_character()`似乎真的很简单，只要返回输入值就可以了。问题是同一个字符串有多种表示编码方式。在继续探讨之前，需要厘清一些概念：  
...
> 字符集是多个字符的集合，字符集种类较多，每个字符集包含的字符个数不同，常见字符集有：ASCII字符集、ISO 8859字符集、GB2312字符集、BIG5字符集、GB18030字符集、Unicode字符集等。ASCII可以非常好地表示英文字符，因为它就是美国信息交换标准代码(American Standard Code for Information Interchange)的缩写。Unicode是国际组织制定的可以容纳世界上所有文字和符号的字符编码方案。   
...
在R中，我们可以使用`charToRaw()`函数获得一个字符串的底层表示（underlying representation）：  
```{r}
...
根据需要的是日期型数据（从1970-01-01开始的天数）、日期时间型数据（从1970-01-01开始的秒数）还是时间型数据（从午夜开始的描述），我们可以在3中解析函数之间进行选择。在没有使用任何附加参数时调用，具体情况如下：  
...
* `type_convert()`函数在一个数据框的所有字符列上应用启发式解析过程：  
```{r}
df <- tribble(
  ~x,~y,
  "1","1.21",
...
* 如果正在读取一个非常大的文件，那么应该将`n_max`设置为一个较小的数，比如10,000或者100,000，这可以让我们在解决常见问题时加快重复试验的过程。

## `readxl`  

要想将其他类型的数据导入 R 中，可以先从下列的`tidyverse`包开始。对矩形数据来说：  

* `haven`可以读取SPSS、Stata和SAS文件:  
![](images\5.png)
...
经常会在读取Excel文件时遇到的一个问题是,有些人喜欢在表格的前面几行或最后几行添加一些无关真正分析内容元数据（metadata），比如在`deaths.xlsx`这个Excel文件中：  
![](images/9.png)
