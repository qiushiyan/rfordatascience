...
```{r, echo = FALSE}
knitr::opts_chunk$set(message = TRUE, out.width = "90%", cache = TRUE)
```
...
gapminder_model
```
Then using `broom` functions to generate "tidy" model summaries:  
...
There are several remaining useful functions in `tidyr` than cannot be categorized.  
