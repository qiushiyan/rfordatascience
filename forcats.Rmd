...
To demonstrate how to collpase multiple levels of a factor, we will start with `fct_count()` to count factor levels. It's basically a variant of `dplyr::count()`, taking a factor (factor) as its first argument instead of a data frame, which makes it a nice function in `mutate()`. 
...
`fct_lump(f, n, prop)` lumps together least / most common factor levels into a new level "other". If both `n` and `prop` are missing, `fct_lump()` lumps together the least frequent levels into "other", while ensuring that "other" is still the smallest level. It's particularly useful in conjunction with `fct_inorder()`.
...
When a factor has missing values, these `NA`s will not be listed as a valid level. Though in some cases `NA` in a factor could be meaningful:  
...
## Combining factors {#forcats-combine}
